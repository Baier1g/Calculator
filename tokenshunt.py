...
def trigon(function,input,deg_mode):
    if deg_mode == 1: # Radians
        ...
        if function == "sin":
            return math.degrees(math.sin(input))
        elif function == "cos":
            return math.degrees(math.cos(input))
        elif function == "tan":
            return math.degrees(math.tan(input))
...
def tokenize(in_str):
    ...
    while len(chars) != 0:
        ...
        elif char in operator_info.keys() or char in ["(", ")"] and output[-1].isalpha() if output[-1] != None else True:
            output.append(buf) if buf != "" else False
            buf = ""
            # Add char to output
            output.append(char)
        
        # Same as the others, but for FURTHER functionality
        else:
            if state != "func":
                output.append(buf) if buf != "" else False
                buf = ""
    ...
    while len(tokens) != 1:
            ...
            while len(operators) != 0:
                ...
                if operators[-1] not in ["(",")"]:
                    ...
                    if operator_info[operators[-1]].precedence > operator_info[tok].precedence:
                        satisfied = True
                    # Sets satisfied to True if the precedence of tok and the previous element are the same AND if the previous element have left associativity
        ...
        # Adds start parentheses to the operator list and if tok is an end parentheses, it will pop from the operators list until it encounters a start parentheses
            ...
            while len(operators) != 0 and operators[-1] != "(":
                output.append(operators.pop())
            # Pop the operators list to get rid of the start parentheses
            operators.pop()
        else:
            operators.append(tok)
...
def parse_tree(postfix_expression):
    ...
    for token in postfix_expression:
        if token.isdigit() or is_dot(token):
            node = Node(token)
            stack.append(node)
        # If the stack has more than 2 elements, it will pop the 2 most recent nodes as right and left
        elif len(stack) > 1:
